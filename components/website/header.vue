  ...
  <div class="container-fluid p-0">
    <div class="d-flex justify-content-between pl-3 pr-3 pt-3">
      ...
      <div class="text-right header-right align-self-center">
        ...
        <button
          @click="open_menu($event)"
          class="btn btn-hov menu d-none material-icons dropdown-btn p-4"
        >menu</button>
      </div>
    </div>
    <div class="w-100 text-center mt-2 bd-bottom d-none">
      <nuxt-link
        :to="item.href"
        class="btn btn-hov mr-2"
        v-for="(item, i) in links2"
        :key="i"
      >{{item.title}}</nuxt-link>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      links: [
        {
          title: "Sign In",
          class: "text-dark border text pl-3 pr-3 bd-radius-0 mr-4",
          href: "./login",
        },
        {
          title: "Join Now!",
          class: "btn-primary box-shadow bd-radius-0 mr-4",
          href: "./join",
        },
      ],
  ...
  methods: {
    dropdown(e, list) {
      let { html, instance } = this.component(dropdown_list, {
        e,
        list,
        width: 250,
      });
