...
<div class="container p-0">
    <div class="w-100 text-right pt-4 pb-4">
        <button class="btn text-sm bd-radius-0 bd-right">
            <span class="d-flex align-items-center">
                ...
                <span class="text-dark font-weight-bold">{{account.username}}</span><br>
            </span>
        </button>
    ...
    <div class="w-100 text-center mt-2 bd-bottom">
        <button @click="change_tab(i)" class="btn btn-hov mr-2" :class="active == i ? 'font-weight-bold bd-bottom-blue' : ''" v-for="(item, i) in links" :key="i">{{item.title}}</button>
    </div>
</div>
</template>
...
export default {
    ...
    methods: {
        logout() {
            this.eraseCookie("mywebsite.registration")
            this.$router.push("/")
        },
        change_tab(i) {
            this.active=i
